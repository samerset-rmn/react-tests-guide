...
Функция `getByRole` используется для поиска элемента с явной ARIA ролью и описанием. Но если у элемента нет описания или он не доступен для пользователя, эта функция ничего не найдет. Способы решения:
...
- Обернуть проверку после асинхронного действия в `await waitFor()`. Например, очистились ли поля формы после завершения асинхронного `onSubmit`? **Такой метод наиболее предпочтителен**.
